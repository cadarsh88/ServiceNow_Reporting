      ...
      link: function (scope, element) {
        ...
        scope.drawGlobalOverview = function () {
          ...
          var item_block = items.selectAll('.item-block-year')
            ...
            .on('mouseover', function(d) {
              ...
              // Construct tooltip
              var tooltip_html = '';
              tooltip_html += '<div><span><strong>Total patches tracked:</strong></span>';

              var patts = parseInt(d.total, 10);
              tooltip_html += '<span>' + patts + '</span></div>';
              ...
              if ( d.summary.length <= 5 ) {
                for ( var i = 0; i < d.summary.length; i++ ) {
                  tooltip_html += '<div><span><strong>' + d.summary[i].ServerName + '</strong></span>';
                  tooltip_html += '<span>' + scope.formatTime(d.summary[i].Status) + '</span></div>';
                };
              } else {
                for ( var i = 0; i < 5; i++ ) {
                  tooltip_html += '<div><span><strong>' + d.summary[i].ServerName + '</strong></span>';
                ...
                var other_projects_sum = d.summary.length;
                /*for ( var i = 5; i < d.summary.length; i++ ) {
                  other_projects_sum =+ d.summary[i].count;
                };*/
        ...
        scope.drawYearOverview = function () {
          ...
          items.selectAll('.item-circle')
            ...
            .on('mouseover', function (d) {
              ...
              // Construct tooltip
              var tooltip_html = '';
              tooltip_html += '<div class="header"><strong>' + (d.total ? d.total : 'No ') + 'Patches tracked</strong></div>';
              tooltip_html += '<div>on ' + moment(d.date).format('dddd, MMM Do YYYY') + '</div><br>';

              // Add summary to the tooltip
              angular.forEach(d.summary, function (d) {
                tooltip_html += '<div><span><strong>' + d.ServerName + '</strong></span>';
                tooltip_html += '<span>' + d.Status + '</span></div>';
              });
        ...
        scope.drawMonthOverview = function () {
          ...
          item_block.selectAll('.item-block-rect')
            ...
            .on('mouseover', function(d) {
              ...
              // Construct tooltip
              var tooltip_html = '';
              // tooltip_html += '<div class="header"><strong>' + d.ServerName + '</strong></div><br>';
              tooltip_html += '<div><strong>' + (d.total ? d.total : 'No ') + 'Patches tracked</strong></div><br>';
        ...
        scope.drawWeekOverview = function () {
          ...
          item_block.selectAll('.item-block-rect')
            ...
            .attr('x', function (d) {
              ...
              var offset = parseInt(d3.select(this.parentNode).attr('offset'));
              itemScale.domain([0, total]);
              d3.select(this.parentNode).attr('offset', offset + itemScale(d.total));
              return offset;
            })
            .attr('width', function (d) {
              var total = parseInt(d3.select(this.parentNode).attr('total'));
            ...
            .on('mouseover', function(d) {
              ...
              var total = parseInt(d3.select(this.parentNode).attr('total'));
              itemScale.domain([0, total]);
              var x = parseInt(d3.select(this).attr('x')) + itemScale(d.total) / 4 + tooltip_width / 4;
              while ( width - x < (tooltip_width + tooltip_padding * 3) ) {
                x -= 10;
              }
        ...
        scope.drawDayOverview = function () {
          ...
          items.selectAll('.item-block')
            ...
            .attr('y', function (d) {
              return (projectScale(d.ServerName) + projectScale.rangeBand() / 2) - 15;
            })
            .attr('width', function (d) {
            ...
            .on('mouseover', function(d) {
              ...
              var tooltip_html = '';
              tooltip_html += '<div class="header"><strong>' + d.ServerName + '</strong><div><br>';
              // tooltip_html += '<div><strong>' + (d.ScheduleStart ? scope.formatTime(d.value) : 'No time') + ' tracked</strong></div>';
          ...
          labels.selectAll('.label-project')
            ...
            .on('mouseenter', function (project) {
              ...
              items.selectAll('.item-block')
                ...
                .ease('ease-in')
                .style('opacity', function (d) {
                  return (d.ServerName === project) ? 1 : 0.1;
                });
            })
            .on('mouseout', function () {
              if ( in_transition ) { return; }
